...
const MSG_TO_PLNASSIST_TARGET: &str = "plnassist";
...
const MSG_TO_PLNASSIST_SIGNATURE_VERIFICATION: &str = "signature_verification";
...
pub(super) extern "C" fn cb_msg(
    ...
    match &target as _ {
        ...
        MSG_TO_PLNASSIST_TARGET => handle_msg_to_plnassist(id, content, len),
        _ => PluginReturn::new(
...
fn handle_msg_to_plnassist(id: String, content: *const c_void, len: usize) -> PluginReturn {
    let s = early_return_value!(
        std::str::from_utf8(unsafe { std::slice::from_raw_parts(content as _, len) }),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg string"
    );
    let msg_to_plnassist = early_return_value!(
        serde_json::from_str::<MsgToPLNAssist>(s),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg '{}'",
        s
    );
    match &msg_to_plnassist.r#type as &str {
        MSG_TO_PLNASSIST_SIGNATURE_VERIFICATION => request_plugin_sign(id, msg_to_plnassist),
        t => PluginReturn::new(
            errno::ERR_CALLBACK_TARGET_TYPE,
            &format!(
                "Unknown target type '{}' for target {}",
                t, MSG_TO_PLNASSIST_TARGET
            ),
        ),
    }
}

fn request_plugin_sign(id: String, msg_to_plnassist: MsgToPLNAssist) -> PluginReturn {
    let signature_data = early_return_value!(
        std::str::from_utf8(&msg_to_plnassist.data),
        ERR_CALLBACK_INVALID_MSG,
