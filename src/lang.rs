...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_plnassist() {
            if s.contains("PLNAssist")
                && !name.starts_with("upgrade_plnassist_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("PLNAssist") {
                    s = s.replace("PLNAssist", &app_name);
                } else {
                    // https://github.com/plnassist/plnassist-server-pro/issues/845
                    // If app_name contains "PLNAssist" (e.g., "PLNAssist-Admin"), we need to avoid
                    // replacing "PLNAssist" within the already-substituted app_name, which would
                    // cause duplication like "PLNAssist-Admin" -> "PLNAssist-Admin-Admin".
                    //
