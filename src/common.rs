...
pub const PORTABLE_APPNAME_RUNTIME_ENV_KEY: &str = "PLNASSIST_APPNAME";
...
pub fn global_init() -> bool {
    crate::local_input_priority::start();
    #[cfg(target_os = "linux")]
    {
...
// Because the url is always `https://api.plnassist.com/version/latest`.
#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let (request, url) =
        hbb_common::version_check_request(hbb_common::VER_TYPE_PLNASSIST_CLIENT.to_string());
...
#[inline]
pub fn is_plnassist() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("PLNAssist")
}
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "https://admin.plnassist.com".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    let url = url.to_ascii_lowercase();
    url.contains("plnassist.com/") || url.ends_with("plnassist.com")
}
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/PLNAssist.app/Contents/MacOS/PLNAssist", "XPC_SERVICE_NAME=com.carriez.PLNAssist_server"
...
pub type PLNAssistInterval = ThrottledInterval;

#[inline]
pub fn plnassist_interval(i: Interval) -> ThrottledInterval {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_PLNAssist_interval() {
        ...
        for maker in base_intervals.into_iter() {
            ...
            let mut timer = plnassist_interval(maker());
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_PLNAssist_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];
    ...
    #[test]
    fn test_is_public() {
        // Test URLs containing "plnassist.com/"
        assert!(is_public("https://plnassist.com/"));
        assert!(is_public("https://www.plnassist.com/"));
        assert!(is_public("https://api.plnassist.com/v1"));
        assert!(is_public("https://API.PLNASSIST.COM/v1"));
        assert!(is_public("https://plnassist.com/path"));

        // Test URLs ending with "plnassist.com"
        assert!(is_public("plnassist.com"));
        assert!(is_public("https://plnassist.com"));
        assert!(is_public("https://PLNAssist.com"));
        assert!(is_public("http://www.plnassist.com"));
        assert!(is_public("https://api.plnassist.com"));
        ...
        assert!(!is_public("https://plnassist.computer.com"));
        assert!(!is_public("plnassist.comhello.com"));
    }
    ...
    fn test_should_use_tcp_proxy_for_api_url() {
        ...
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.plnassist.com/api/login",
            "https://admin.plnassist.com"
        ));
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.example.com/api/login",
            "not a url"
