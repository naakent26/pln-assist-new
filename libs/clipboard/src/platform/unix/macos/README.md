...
1. Use a temporary file `/tmp/plnassist_<uuid>` as a placeholder in the pasteboard.
...
2. Create temp file in `/tmp/.plnassist_*`.

### `paste_observer.rs`

Use `fsevent` to observe the paste operation with the source file `/tmp/.plnassist_*`.
