...
class PlatformFFI {
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libplnassist.so')
        : isLinux
            ? DynamicLibrary.open('libplnassist.so')
            : isWindows
                ? DynamicLibrary.open('libplnassist.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibplnassist.dylib") :
  ...
  void _startListenEvent(RustdeskImpl plnassistImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = plnassistImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
