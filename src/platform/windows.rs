    ...
    loop {
        ...
        if !sids.contains(&session_id) || !is_share_rdp() {
            ...
            if session_id != current_active_session {
                ...
                // https://github.com/plnassist/plnassist/discussions/10039
...
pub fn portable_service_logon_helper_paths() -> Option<(PathBuf, PathBuf)> {
    ...
    let dir = user_dir
        ...
        .join("AppData")
        .join("Local")
        .join("plnassist-sciter");
    let dst = dir.join("plnassist.exe");
    Some((dir, dst))
}
...
fn get_after_install(
    ...
    // https://github.com/plnassist/plnassist/commit/f4bdfb6936ae4804fc8ab1cf560db192622ad01a
...
copy /Y \"{tmp_path}\\Uninstall {app_name}.lnk\" \"{start_menu}\\\"
    ...
    // Note: without if exist, the bat may exit in advance on some Windows7 https://github.com/plnassist/plnassist/issues/895
    let dels = format!(
        "
...
fn run_cmds(cmds: String, show: bool, tip: &str) -> ResultType<()> {
    ...
    // https://github.com/plnassist/plnassist/issues/6786#issuecomment-1879655410
...
pub fn get_custom_client_staging_dir() -> PathBuf {
    get_public_base_dir()
        .join("PLNAssist")
        .join("PLNAssistCustomClientStaging")
}
...
/// - Deleting directories under a public location (e.g., C:\\ProgramData\\PLNAssist) is
...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging PLNAssist.
...
pub fn create_shortcut(id: &str) -> ResultType<()> {
    ...
    // https://github.com/plnassist/plnassist/issues/13735
    // Replace ':' with '_' for filename since ':' is not allowed in Windows filenames
    // https://github.com/plnassist/hbb_common/blob/8b0e25867375ba9e6bff548acf44fe6d6ffa7c0e/src/config.rs#L1384
...
mod cert {
    ...
    extern "C" {
        fn DeletePLNAssistTestCertsW();
    }
    pub fn uninstall_cert() -> ResultType<()> {
        unsafe {
            DeletePLNAssistTestCertsW();
        }
        Ok(())
    }
}
...
pub fn get_unicode_from_vk(vk: u32) -> Option<u16> {
    ...
    let len = unsafe {
        ...
        // refs: https://github.com/plnassist-org/rdev/blob/25a99ce71ab42843ad253dd51e6a35e83e87a8a4/src/windows/keyboard.rs#L115
        let press_state = 129;
...
reg add {subkey} /f /v EstimatedSize /t REG_DWORD /d {size}
    ...
    // 1. There may be some other processes like `plnassist --connect` are running.
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                // Match files like plnassist-*.msi or plnassist-*.exe
                if file_name.starts_with("plnassist-")
...
pub fn try_kill_plnassist_main_window_process() -> ResultType<()> {
    // Kill plnassist.exe without extra arg, should only be called by --server
    ...
    bail!("failed to find plnassist main window process");
}
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    ...
    unsafe {
        ...
        if res != 0 {
            bail!("Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_plnassist.log for more details.");
        } else {
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\PLNAssist\PLNAssist.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\PLNAssist\PLNAssist.exe"
    // ProcessId=10136
    let mut pids = Vec::new();
