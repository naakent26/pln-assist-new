...
void msgBox(SessionID sessionId, String type, String title, String text,
  ...
  submit() {
    ...
    if (onSubmit != null) {
      ...
      // https://github.com/plnassist/plnassist/blob/5e9a31340b899822090a3731769ae79c6bf5f3e5/src/ui/common.tis#L263
...
Future<bool> restoreWindowPosition(WindowType type,
  ...
  if (bind
      .mainGetEnv(key: "DISABLE_PLNASSIST_RESTORE_WINDOW_POSITION")
      .isNotEmpty) {
    return false;
  }
  if (type != WindowType.Main && windowId == null) {
  ...
  if (lpos == null) {
    ...
    switch (type) {
      case WindowType.Main:
        ...
        // See https://github.com/plnassist/plnassist/blob/9b9276e7524523d7f667fefcd0694d981443df0e/flutter/macos/Runner/Base.lproj/MainMenu.xib#L333
      ...
      default:
        ...
        // https://github.com/plnassist/plnassist/blob/317639169359936f7f9f85ef445ec9774218772d/flutter/lib/utils/multi_window_manager.dart#L163
        break;
    }
    return true;
  }
      ...
      if (lpos.isMaximized == true) {
        ...
        if (isWindows) {
          if (storeSize) {
            ...
            // See known issue in https://github.com/plnassist/plnassist/pull/9840
  ...
  if (uri.authority.isEmpty &&
    ...
    if (isAndroid || isIOS) {
      ...
      if (!allowDeepLinkServerSettings) {
        debugPrint(
            "Ignore plnassist://config because $kOptionAllowDeepLinkServerSettings is not enabled.");
    ...
    if (isAndroid || isIOS) {
      ...
      if (!allowDeepLinkPassword) {
        debugPrint(
            "Ignore plnassist://password because $kOptionAllowDeepLinkPassword is not enabled.");
        // Keep the user-facing error generic; detailed rejection reason is in debug logs.
        // Delay toast to avoid missing overlay during cold-start deeplink handling.
        Timer(Duration(seconds: 1), () {
          showToast(translate('Failed'));
    ...
    // plnassist://<connect-id>
    // plnassist://<connect-id>/r
    // plnassist://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
Future<void> onActiveWindowChanged() async {
  ...
  if (rustDeskWinManager.getActiveWindows().isEmpty) {
    ...
    try {
      ...
      debugPrint("Start closing PLNAssist...");
      ...
      if (isMacOS) {
        ...
        // 2024-11-11 11:41:11.546 PLNAssist[90272:2567686] Failed to create a FlutterPlatformMessageResponseHandle (2)
        // embedder.cc (2672): 'FlutterEngineSendPlatformMessage' returned 'kInvalidArguments'. Invalid engine handle.
        // 2024-11-11 11:41:11.565 PLNAssist[90272:2567686] Failed to send message to Flutter engine on channel 'flutter/lifecycle' (2).
        // ```
        periodic_immediate(
...
class ServerConfig {
  ...
  /// decode from shared string (from user shared or plnassist-server generated)
  /// also see [encode]
...
const kMacOSPermChannel = MethodChannel("org.plnassist.plnassist/host");
...
setNewConnectWindowFrame(int windowId, String peerId, int preSessionCount,
  ...
  if (screenRect == null) {
    ...
    // https://github.com/plnassist/plnassist/discussions/8825
...
Widget loadPowered(BuildContext context) {
  ...
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse('https://plnassist.com'));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
      ...
      children: [
        Align(
          ...
          style: TextStyle(
            color: Colors.red,
            fontSize:
                18, // https://github.com/plnassist/plnassist-server-pro/issues/261
            fontWeight: FontWeight.bold,
          ),
...
bool whitelistNotEmpty() {
  // https://plnassist.com/docs/en/self-host/client-configuration/advanced-settings/#whitelist
...
// https://github.com/plnassist/plnassist/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L385
// https://github.com/plnassist/plnassist/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L399
//
// @platforms macos
...
String decode_http_response(http.Response resp) {
  try {
    // https://github.com/plnassist/plnassist-server-pro/discussions/758
