...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "plnassist".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/plnassist-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "PLNAssist wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "PLNAssist wants to uninstall the plugin",
    )
}
